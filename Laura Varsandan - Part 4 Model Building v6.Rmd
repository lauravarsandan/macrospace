...
Firstly, let's remove the rows with negative values

```{r}
RT_train <- RT_train[sales_per_unit>=0]
RT_test <- RT_test[sales_per_unit>=0]
```

Next it looks like we need to take log transformations of our target variable:
...
```{r}
boxplot(RT_train$log_sales_per_unit)
hist(RT_train[sales_per_unit<50,sales_per_unit])
# taking out the outliers
RT_train<-RT_train[sales_per_unit<50]
RT_test<-RT_test[sales_per_unit<50]

RT_train[,mean(sales_per_unit)]
RT_train[,mean(log_sales_per_unit)]

unique(is.finite(RT_train$sales_per_unit))
table(is.finite(RT_train$log_sales_per_unit))
...
RT_test$RT2_predictions_log <- log(RT2_predictions)
head(RT_test)
```
Visualize the model
...
```{r}
RT2 <- tree(sales_per_unit ~ family + Lat + Long + no_items.y + transactions.y + tran_on_wknd,data=RT_train, mindev=0.0001)
summary(RT2)
```
...
First let's check if there are any negative sales_per_unit which shouldn't really present in our dataset as those values are not valid. 

```{r}
nrow(train_base_results[sales_per_unit<0])
nrow(test[sales_per_unit<0])
```

it looks like we do have some negative values. Let's exclude them from the dataset. 

```{r}
train_base_results <- train_base_results[sales_per_unit>=0]
test <- test[sales_per_unit>=0]
```

Let's also look at the distribution of our residuals:

```{r}
boxplot(train_base_results$base_residuals)
```

It looks like we migh need the log transformation:

```{r}
train_base_results <- train_base_results[,log_base_residuals:=log(base_residuals)]
head(train_base_results)
```
...
```{r}
RT4 <- tree(base_residuals ~ month + dow , data=train_base_results, mindev=0.0001)
summary(RT4)
```
...
test$RT4_predictions <- RT4_predictions
...
```{r}
ggplot(test, aes(RT4_predictions,sales_per_unit)) + geom_point(aes(color = family)) + 
  theme_bw() + labs(title="Scatterplot") +facet_wrap( ~ family)

```

We can see that outliers are making the vizualisation difficult. Let's exclude them from the visualization, but first let's find out which ones they are. 

```{r}
#are there negative sales per unit? They might be skewing our predictions

boxplot(test$sales_per_unit)
boxplot(log(test$sales_per_unit))
```
