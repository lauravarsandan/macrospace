...
```{r}
train <- class_merged_all_features[date<='2017-05-31']
...
Let's look at the values of our target variable:


```{r}
boxplot(RT_test$sales_per_unit)
```

It looks like it needs a log transformation [explain in more detail why]

```{r}
RT_train <- RT_train[,log_sales_per_unit:=log(sales_per_unit)]
RT_test <- RT_test[,log_sales_per_unit:=log(sales_per_unit)]
```

Remove the NaNs and Infinite Values from the log transformations

```{r}
RT_train <- RT_train[!is.nan(log_sales_per_unit)]
RT_test <- RT_test[!is.nan(log_sales_per_unit)]

# check
table(is.finite(RT_train$log_sales_per_unit))

# remove the infinite values
RT_train <- RT_train[is.finite(log_sales_per_unit)]
RT_test <- RT_test[is.finite(log_sales_per_unit)]
```


Let's vizualise the results now:

```{r}
boxplot(RT_test$log_sales_per_unit)
```
...
```{r}
RT2 <- tree(log_sales_per_unit ~ family ,data=RT_train, mindev=0.0001)
summary(RT2)
```
...
```{r}
RT_test$RT2_predictions <- exp(RT2_predictions)
head(RT_test)
```
Visualize the model
...
#variance explained
mean_test_sales_per_unit <- RT_test[,mean(sales_per_unit)]
TV_model3 <- sum((RT_test$sales_per_unit-mean_test_sales_per_unit)^2)
...
```{r}
RT2 <- tree(log_sales_per_unit ~ family + Lat + Long + no_items.y + transactions.y + tran_on_wknd,data=RT_train, mindev=0.0001)
summary(RT2)
```
...
```{r}
RT2 <- tree(log_sales_per_unit ~ family + transactions.y + month,data=RT_train, mindev=0.0001)
summary(RT2)
```
...
```{r}
RT2 <- tree(log_sales_per_unit ~ family + transactions.y + tran_on_wknd + month + nat_hol_fctr + wage_factor,data=RT_train, mindev=0.0001)
summary(RT2)
```
...
```{r}
RT2 <- tree(log_sales_per_unit ~ family + transactions.y + tran_on_wknd + month + year + nat_hol_fctr + wage_factor,data=RT_train, mindev=0.00001)
summary(RT2)
```
...
```{r}
write.csv(RT_train,"RT_train.csv")
write.csv(RT_test,"RT_test.csv")
...
```{r}
month_year <- unique(train[date>='2015-01-01',c("month_year")])
...
for (i in 1:length(month_year)){
      m=month_year[i]
      max_date <- train[month_year==m,max(date)]
      d <- max_date - 60 #include data only until when you would start doing the prediction
      head(train[date<=max_date])
      base_data <- train[date<=d & year>=year(max_date)-1]
      unique(base_data$month_year)
      train_bm <- base_data[,mean(sales_per_unit),by=c('store_nbr','class','year')]
      names(train_bm)[4]<-"base_prediction"
      if (month(max_date) %in% c(1,2)) {
        train_bm_new <- train_bm[year==year(max_date)-1]
        train_bm_new$year <- year(max_date)
      } else  train_bm_new <- train_bm
      month_year_pred <- train[month_year==m]
      month_year_pred <- merge(month_year_pred,train_bm_new, by=c("store_nbr","class","year"),all.x=TRUE)
      #remove nulls
      month_year_pred <- month_year_pred[!is.na(base_prediction)]
      #store the data
      if (i==1){
          train_base_results <- month_year_pred
      } else train_base_results <- rbind(train_base_results,month_year_pred)
}
```

Let's see how the model performs on the train dataset

Since we removed some rows when they were new additions, let's see what % of data we kept


```{r}
train_base_results <- fread("train_base_results.csv")
nrow(train_base_results)/nrow(train)
```

We still kept the majority of lines, which means that we can use the results

```{r}
#variance explained
mean_train_sales_per_unit <- train[,mean(sales_per_unit)]
TV_train <- sum((train_base_results$sales_per_unit-mean_train_sales_per_unit)^2)
VE_train_base <- sum((train_base_results$base_prediction-mean_train_sales_per_unit)^2)
R2_train_base<-VE_train_base/TV_train
R2_train_base

#RMSE
RMSE_train_base<- sqrt(sum((train_base_results$base_prediction-train_base_results$sales_per_unit)^2)/nrow(train_base_results))
RMSE_train_base 
```

Let's add the new target variable: the residuals from the base model:

```{r}
test$base_residuals <- test$sales_per_unit - test$base_prediction
#train_base_results$base_residuals <- train_base_results$sales_per_unit- train_base_results$base_prediction
train_base_results <- train_base_results[,base_residuals:=sales_per_unit-base_prediction]
head(train_base_results)
```
Let's visualize the distributions
 
```{r}
boxplot(train_base_results$base_residuals)
```

Again it looks like the target variable needs a log tranformation to improve the variance. However, we have negative values, so we will have to adjust the transformation by adding a constant that will make the smallest value of base_residuals positive. We will set the same constant for both the train and test. 

```{r}
train_base_results[,min(base_residuals)]
test[,min(base_residuals)]
```

Let's therefore set the constant a=865 so that a+min(train$base_residual). 

```{r}
a=865
train_base_results <- train_base_results[,base_residuals_tran:=base_residuals+a]
test <- test[,base_residuals_tran:=base_residuals+a]

# take the log of the transformed

train_base_results <- train_base_results[,base_residuals_log:=log(base_residuals_tran)]
test <- test[,base_residuals_log:=log(base_residuals_tran)]
...
```{r}
boxplot(train_base_results$base_residuals_log)
```




## Model 4: RT with all other features except class and store.

First let's add all the factors needed: month, weekdays. 

```{r}
train_base_results <- train_base_results[,month:=as.factor(month)]
test <- test[,month:=as.factor(month)]
train_base_results <- train_base_results[,date:=as.Date(date)]
train_base_results <- train_base_results[,dow:=as.factor(weekdays(date))]
test <- test[,dow:=as.factor(weekdays(date))]
test <- test[,day_of_month:=as.factor(day_of_month)]
train_base_results <- train_base_results[,day_of_month:=as.factor(day_of_month)]
train_base_results <- train_base_results[family=="GROCERY II",family:="GROCERY I"]
test <- test[family=="GROCERY II",family:="GROCERY I"]

#is_weekday
unique(test$is_weekday)
unique(train_base_results$is_weekday)
train_base_results <- train_base_results[dow %in% c("Monday","Tuesday","Wednesday","Thursday","Friday"),is_weekday:="1"]
train_base_results <- train_base_results[dow %in% c("Saturday","Sunday"),is_weekday:="0"]
train_base_results <- train_base_results[,is_weekday:=as.factor(is_weekday)]
test <- test[dow %in% c("Monday","Tuesday","Wednesday","Thursday","Friday"),is_weekday:="1"]
test <- test[dow %in% c("Saturday","Sunday"),is_weekday:="0"]
test <- test[,is_weekday:=as.factor(is_weekday)]

#perc_on_prom
train_base_results <- train_base_results[,perc_on_prom:=items_onpromotion/no_items.x]
test <- test[,perc_on_prom:=items_onpromotion/no_items.x]


head(train_base_results)
head(test)

boxplot(train_base_results$perc_on_prom)

```



Fit the model with time dimensions as the base model already takes into account the store and class characteristics.

```{r}
RT4 <- tree(base_residuals_log ~ month + dow + nat_hol_fctr + perc_on_prom, data=train_base_results, mindev=0.001)
summary(RT4)
```
...
```{r}
plot(RT4, main="Model3 Regression Tree on the Class")
text(RT4, cex=.8, pretty=FALSE)
```
Predict for the test set

```{r}
RT4_predictions <- predict(RT4,test)
```

Save the predictions

```{r}
#train_base_results$RT4_predictions <- predict(RT4,train_base_results)
test$RT4_predictions <- exp(RT4_predictions)-a
test <- test[,RT4_predictions:=base_prediction+RT4_predictions]
head(test)
```
...
#variance explained
mean_test_sales_per_unit <- test[,mean(sales_per_unit)]
TV_model4 <- sum((test$sales_per_unit-mean_test_sales_per_unit)^2)
VE_model4 <- sum((test$RT4_predictions-mean_test_sales_per_unit)^2)
R2_model4<-VE_model4/TV_model4
R2_model4

#RMSE
RMSE_model4<- sqrt(sum((test$RT4_predictions-test$sales_per_unit)^2)/nrow(test))
RMSE_model4 
```
...
```{r}
ggplot(test, aes(log(RT4_predictions),log(sales_per_unit))) + geom_point(aes(color = family)) + 
  theme_bw() + labs(title="Scatterplot") +facet_wrap( ~ family)

```
We can see that most of the families display similar pattern of over estimation, with the exception of Automative. Let's add the family to the RT and see what happens.

Let's see how the predictions vary depending on the stores.

```{r}
ggplot(test, aes(log(RT4_predictions),log(sales_per_unit))) + geom_point(aes(color = store_nbr)) + 
  theme_bw() + labs(title="Scatterplot") +facet_wrap( ~ store_nbr)
```

We can see similar trend as for the families sections. Except, store 3 and 5 have some clear outliers. 

Let's look at the day of the month. 


```{r}
ggplot(test, aes(log(RT4_predictions),log(sales_per_unit))) + geom_point(aes(color = day_of_month)) + 
  theme_bw() + labs(title="Scatterplot") +facet_wrap( ~ day_of_month)
```

With this the graphs vary a lot more and there are some interesting patterns. Let's try day of week


```{r}
ggplot(test, aes(log(RT4_predictions),log(sales_per_unit))) + geom_point(aes(color = dow)) + 
  theme_bw() + labs(title="Scatterplot") +facet_wrap( ~ dow)
```


Clear distinct patterns between the different week days. Let's try and replace day of week with a wage_factor. Let's replace the dow with an is_weekend_factor and see how it behaves. Now all the graphs during the weekdays look more similar. 


Let's see how the numerical factors on the test set correlate with our target variable. 

```{r}
numerical <- test[,c("sales_per_unit","RT4_predictions","wage_factor","nat_hol_fctr","reg_hol_fctr","items_onpromotion","no_perishable_items","dcoilwtico.y","tran_on_wknd")]
...
No other continous variable is strongly correlated with sales per unit. 

Let's investigate where the residuals lie. 

```{r}
test <- test[,RT4_residuals:=sales_per_unit-RT4_predictions]
write.csv(test,"best_model_test_results.csv")
...
```{r}
avg_rez <- test[,mean(RT4_residuals),by=c("family","dow")]
names(avg_rez)[3] <- "average_residual"
```

```{r}
ggplot(avg_rez, aes(dow, family)) + geom_tile(aes(fill = average_residual) , colour = "white") + scale_fill_gradient2(low = "red", midpoint= 0, high = "steelblue")
```                                               

Let's take a subset of the families with the highest residuals

```{r}
subset_families <- unique(avg_rez[average_residual>5 | average_residual<5,"family"])
subset_families
```

Let's save the output of the final model but also the actual values for the month before

```{r}
write.csv(test,"test.csv")
m=unique(test$month_year)
m
last_month <- train[month_year==m-2]
write.csv(last_month,"last_month.csv")
```
